...
async def test_login_user(async_client: AsyncClient, create_user) -> None:
    ...
    await create_user(username=USER_TRUE_USERNAME, email=USER_TRUE_EMAIL, password=USER_TRUE_PASSWORD)
...
async def test_get_user(async_client: AsyncClient, create_user) -> None:
    ...
    user = await create_user(username=USER_TRUE_USERNAME, email=USER_TRUE_EMAIL, password=USER_TRUE_PASSWORD)
...
async def test_get_all_users(async_client: AsyncClient, create_user) -> None:
    ...
    await create_user(email=USER_TRUE_EMAIL, username=USER_TRUE_USERNAME, password=USER_TRUE_PASSWORD)
    await create_user(
...
async def test_update_user(async_client: AsyncClient, create_user, auth_header) -> None:
    ...
    response = await async_client.put(f"/users/{true_user.id}", json=payload, headers=true_header)
    assert response.status_code == 200
    data = response.json()
    assert data["email"] == NEW_EMAIL
    assert data["username"] == NEW_USERNAME

    response_with_false_header = await async_client.put(f"/users/{true_user.id}", json=payload, headers=false_header)
...
async def test_delete_user(async_client: AsyncClient, create_user, auth_header) -> None:
    ...
    response_with_false_header = await async_client.delete(f"/users/{true_user.id}", headers=false_header)
    assert response_with_false_header.status_code == 403
